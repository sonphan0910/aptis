...
export default function ReadingMatchingQuestion({ question, onAnswerChange }) {
            ...
            <Box key={`text-item-${item.id}`} sx={{ mb: 2, display: 'flex', alignItems: 'flex-start', gap: 2 }}>
              ...
              <FormControl size="small" sx={{ minWidth: 200 }}>
                <Select
                  ...
                  displayEmpty
                  renderValue={(selected) => selected ? getOptionDisplayText(selected) : <em>-- Chọn --</em>}
                  sx={{
  ...
  // STRATEGY 1: Try parsing JSON content first (New format)
  ...
  try {
    ...
    if (question.content && (question.content.trim().startsWith('{') || question.content.trim().startsWith('['))) {
      ...
      if (parsedContent.persons && Array.isArray(parsedContent.persons)) {
        console.log('[ReadingMatchingQuestion] JSON content detected and parsed successfully');

        instructionText = parsedContent.instructions || 'Match the following questions with the correct person.';

        personDescriptions = parsedContent.persons.map(p => {
          // Extract letter from "Person A" -> "A"
          const nameMatch = p.name ? p.name.match(/Person\s+([A-Z])/i) : null;
          const letter = nameMatch ? nameMatch[1] : p.name || '?';
    ...
    console.log('[ReadingMatchingQuestion] JSON parse failed, falling back to text parsing:', error);
    // Ignore error and continue to text parsing strategy
  }

  // STRATEGY 2: Fallback to text parsing (Old/Original format)
  if (!isJsonParsed) {
