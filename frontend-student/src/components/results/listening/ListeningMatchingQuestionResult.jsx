...
export default function ListeningMatchingQuestionResult({ answer, question, showCorrectAnswer = true }) {
  ...
  // For LISTENING_MATCHING, answer_json structure is: { [itemId]: optionId }
  ...
  return (
    <Box>
      ...
      {sortedItems.map((item, index) => {
        ...
        return (
          <Box key={`speaker-item-${item.id}`} sx={{ mb: 2 }}>
            <Paper sx={{
              p: 2,
      ...
      <Typography variant="body2" color="text.secondary">
        Hoàn thành {sortedItems.filter(item => {
          const userAnswerOptionId = userAnswers[item.id];
          const userAnswerText = userAnswerOptionId ? optionMap[userAnswerOptionId] : null;
          return userAnswerText === item.answer_text;
        }).length}/{sortedItems.length} speakers
      </Typography>
    </Box>
  );
}